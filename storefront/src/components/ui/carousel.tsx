...
export const Slider: React.FC<SliderProps> = ({
  ...
  const addImgToSlider = useCallback(() => {
    ...
    setSlidesArr((prev: ThumbnailSlide[]) => {
      ...
      if (prev.some((slide) => slide.src === thumnailSrc)) return prev
            ...
            {index === selectedIndex && (
              <AnimatePresence mode="wait">
                <motion.div
                  ...
                  className={cn(
                    "absolute z-[3] w-full h-1 left-0 top-0 dark:bg-white bg-black rounded-full",
                    activeclass
                  )}
                />
              </AnimatePresence>
    ...
    <div className="mt-2 overflow-hidden" ref={emblaThumbsRef}>
      <div className="flex flex-row gap-2">
        {slidesrArr.map((slide, index) => (
          <div
            key={`thumb-${index}`}
            className={`min-w-0 w-full xl:h-24 aspect-auto border-2 ${
              index === selectedIndex
                ? "opacity-100"
