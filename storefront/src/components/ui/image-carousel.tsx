    ...
    >
      <Dialog>
        ...
        <DialogPortal>
          ...
          <DialogContent className="fixed inset-0 z-50 flex flex-col items-center justify-center p-0 ">
            ...
            <div className="relative flex items-center justify-center w-screen h-screen">
                  ...
                  <>
                    ...
                    {showImageControls && (
                      <div className="absolute z-10 flex gap-2 -translate-x-1/2 bottom-4 left-1/2">
                        <button
                          onClick={() => zoomOut()}
                          className="p-2 text-white transition-colors rounded-full cursor-pointer bg-black/50 hover:bg-black/70"
                          aria-label="Zoom out"
                        >
              ...
              <DialogClose asChild>
                <button
                  className="absolute z-10 p-2 text-white transition-colors border rounded-full cursor-pointer top-4 right-4 bg-black/50 hover:bg-black/70"
                  aria-label="Close"
                >
                  <X className="size-6" />
                </button>
...
const Thumb: React.FC<ThumbPropType> = (props) => {
  ...
  return (
    ...
    >
      <button
        onClick={onClick}
        className="relative w-full p-0 overflow-hidden bg-transparent border-0 rounded-md appearance-none cursor-pointer touch-manipulation"
        type="button"
      >
        <div
          className={cn(
      ...
      {showThumbs && thumbPosition === "top" && (
        <div className="mb-4">
          <div className="overflow-hidden" ref={emblaThumbsRef}>
            <div className="flex -ml-3 thumbs-horizontal group">
        ...
        <div ref={emblaRef} className="overflow-hidden rounded-lg">
          <div className="flex -ml-4">
            {images?.map((image, index) => (
              <div
                key={index}
                className="min-w-0 pl-4 shrink-0 grow-0 basis-full"
                role="group"
